...
public class KafkaErrorHandlingConfig {
    ...
    public DefaultErrorHandler errorHandler(KafkaTemplate<String, String> kafkaTemplate) {
        ...
        var recover = new DeadLetterPublishingRecoverer(kafkaTemplate,
                (consumerRecord, e) -> new TopicPartition(consumerRecord.topic() +
                        ".DLT", consumerRecord.partition()));

        var backOff = new ExponentialBackOff(1000L, 2.0);
        backOff.setMaxElapsedTime(10000L);

        DefaultErrorHandler errorHandler = new DefaultErrorHandler(recover, backOff);
        errorHandler.addNotRetryableExceptions(OrderNotFoundException.class, JacksonException.class);
        return errorHandler;
    }
}
